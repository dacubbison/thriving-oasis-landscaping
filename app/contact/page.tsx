...
export default function ContactPage() {
  ...
  return (
    ...
    >
      ...
      <p className="mb-6 text-red-600">Get a free quote for professional Christmas light installation in The Woodlands TX, Kingwood TX, or Montgomery County TX, plus winter landscaping services.</p>
      <form onSubmit={handleSubmit} className="space-y-4 max-w-lg mx-auto">
        <div>
          <label htmlFor="name" className="block mb-1 text-green-800">Name</label>
          <input id="name" type="text" name="name" className="w-full p-2 border rounded" required />
          <ValidationError prefix="Name" field="name" errors={state.errors} />
        </div>
        <div>
          <label htmlFor="email" className="block mb-1 text-green-800">Email Address</label>
          <input id="email" type="email" name="email" className="w-full p-2 border rounded" required />
          <ValidationError prefix="Email" field="email" errors={state.errors} />
        </div>
        <div>
          <label htmlFor="phone" className="block mb-1 text-green-800">Phone</label>
          <input id="phone" type="tel" name="phone" className="w-full p-2 border rounded" required />
          <ValidationError prefix="Phone" field="phone" errors={state.errors} />
        </div>
        <div>
          <label htmlFor="message" className="block mb-1 text-green-800">Message</label>
